...
ILISTFILE="../../../../../MusVar/assets/Targets/Twist_Mouse_Exome_Target_Rev1_GRCm38/Twist_Mouse_Exome_Target_Rev1_7APR20_GRCm38_targets.ilist"
...
MM_GTF="/juno/bic/depot/annotation/M.musculus/ensembl/v83/Mus_musculus.GRCm38.83.gtf"
...
canonicalT=o1 %>% group_by(gene_name) %>% count(transcript_id) %>% arrange(desc(n)) %>% distinct(gene_name,.keep_all=T) %>% pull(transcript_id)
